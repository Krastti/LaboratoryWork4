...
private:
   ...
   * index уже лежит в materialized, то есть если мы вызываем элемент, индекс
   * которого больше последнего индекса элемента в materialized, то данная
...
public:
  ...
  /**
   * Создает бесконечную последовательность по рекуррентному правилу и
   * начальному контексту
   */
  LazySequence(T (*recurrence)(Sequence<T> *), const Sequence<T> &initial);

  /**
   * Создает бесконечную последовательность с заданным размером контекстного
   * окна генератора
   */
